...
export default function DashboardPage(){
 ...
 useEffect(()=>{load();if(!token)return;const socket=new WebSocket(`${api.replace(/^http/,'ws')}/api/v1/events?token=${encodeURIComponent(token)}`);socket.onmessage=()=>load();return()=>socket.close();},[token,risk,status,category]);
 ...
 async function select(id:string){const response=await fetch(`${api}/api/v1/incidents/${id}`,{headers:headers()});if(response.ok)setSelected(await response.json());}
 async function verify(id:string){const response=await fetch(`${api}/api/v1/incidents/${id}/verification`,{method:'PATCH',headers:headers(),body:JSON.stringify({status:'VERIFIED',notes:'Verified during responder review.'})});if(response.ok){setNotice('Incident marked verified.');await select(id);load();}}
 async function retry(report:ProcessingReport){const response=await fetch(`${api}/api/v1/reports/${report.id}/retry`,{method:'POST',headers:headers()});setNotice(response.ok?'Retry queued for provider processing.':report.processing_error||'This report cannot be retried.');load();}
 async function logout(){try{await fetch(`${api}/api/v1/auth/logout`,{method:'POST',headers:headers()});}finally{setToken('');setSelected(null);setIncidents([]);setProcessing([]);setSummary(null);setNotice('Responder session closed.');}}
