...
function exactTimestamp(value:string|undefined){
 if(!value)return 'Submission time unavailable';
 const submittedAt=new Date(value);
 if(Number.isNaN(submittedAt.getTime()))return 'Submission time unavailable';
 return new Intl.DateTimeFormat('en-KE',{dateStyle:'medium',timeStyle:'medium'}).format(submittedAt);
}

function submissionLabel(incident:Incident){
 const channel:{[key:string]:string}={PWA:'Web PWA',WHATSAPP:'WhatsApp',USSD:'USSD',IVR:'Voice IVR'};
 const isVoice=incident.source_type==='IVR'||incident.evidence?.some(evidence=>evidence.evidence_type==='AUDIO_HASH_ONLY');
 return `${channel[incident.source_type]||incident.source_type} · ${isVoice?'Voice report':'Text report'}`;
}
